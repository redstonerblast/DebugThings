...
public class FunctionExHolder {
    ...
    static final FunctionExHolder.ResultConsumer<ServerCommandSource> RESULT_REPORTER = new FunctionExHolder.ResultConsumer<ServerCommandSource>() {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        ...
        for (DataCommand.ObjectType objectType : DataCommand.SOURCE_OBJECT_TYPES) {
            objectType.addArgumentsToBuilder(literalArgumentBuilder, builder -> builder.executes(new FunctionExHolder.Command() {
                @Override
                protected NbtCompound getArguments(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
                    return objectType.getObject(context).getNbt();
                            ...
                            .then(CommandManager.literal("localized").executes(new FunctionExHolder.Command() {
                                        @Override
                                        protected NbtCompound getArguments(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
                                            ...
                                            return FunctionExHolder.getArgument(newPath, objectType.getObject(context));
                                        }
                                    })
                            )
                    .executes(new FunctionExHolder.Command() {
                @Override
                protected NbtCompound getArguments(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
                    return FunctionExHolder.getArgument(NbtPathArgumentType.getNbtPath(context, "path"), objectType.getObject(context));
                }
            })));
        }

        dispatcher.register(
                CommandManager.literal("functionex")
                        .requires(source -> source.hasPermissionLevel(2))
                        .then(CommandManager.argument("name", CommandFunctionArgumentType.commandFunction()).suggests(SUGGESTION_PROVIDER).executes(new FunctionExHolder.Command() {
                            @Nullable
                            @Override
                            protected NbtCompound getArguments(CommandContext<ServerCommandSource> context) {
                                return null;
                            }
                        }).then(CommandManager.argument("arguments", NbtCompoundArgumentType.nbtCompound()).executes(new FunctionExHolder.Command() {
                            @Override
                            protected NbtCompound getArguments(CommandContext<ServerCommandSource> context) {
    ...
    public static ServerCommandSource createFunctionExHolderSource(ServerCommandSource source) {
    ...
    public static <T extends AbstractServerCommandSource<T>> void enqueueAction(
            ...
            FunctionExHolder.ResultConsumer<T> resultConsumer,
    ...
    private static <T extends AbstractServerCommandSource<T>> ReturnValueConsumer wrapReturnValueConsumer(
            T flags, FunctionExHolder.ResultConsumer<T> resultConsumer, Identifier id, ReturnValueConsumer wrapped
    ) {
    ...
    abstract static class Command extends ControlFlowAware.Helper<ServerCommandSource> implements ControlFlowAware.Command<ServerCommandSource> {
        ...
        public void executeInner(
            ...
            if (collection.isEmpty()) {
                throw FunctionExHolder.NO_FUNCTIONS_EXCEPTION.create(Text.of(pair.getFirst()));
            } else {
                NbtCompound nbtCompound = this.getArguments(commandContext);
                ServerCommandSource serverCommandSource2 = FunctionExHolder.createFunctionExHolderSource(serverCommandSource);
                ...
                FunctionExHolder.enqueueAction(
                        collection, nbtCompound, serverCommandSource, serverCommandSource2, executionControl, FunctionExHolder.RESULT_REPORTER, executionFlags
                );
            }
        }
    }
